...
class NERDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # 等价于如下语句
        # item['input_ids'] = self.encodings['input_ids'][idx]  # ['x'] 也可替换为 .x
        # item['attention_mask'] = self.encodings['attention_mask'][idx]  # 此处省略转换为tensor
        # .... 其他在encoding里面的字段
        item = {key: torch.tensor(val[idx]) for key, val in self.encodings.items()}
        if self.labels is not None:
            item['labels'] = torch.tensor(self.labels[idx])
        # 这里item就是encoding的子集，和encoding['字段'][idx]所有字段合在一起一样，不过更方便后续小批量获取
        return item
