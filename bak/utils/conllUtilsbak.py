...
keywords = {'roberta-base': 'Ġ', 'albert-base-v2': '▁', 'distilbert-base-cased': None,
            'bert-base-multilingual-cased': None, 'xlm-roberta-base': '▁'}
...
def encode(lang, tokenizer, kind):
    model_name = tokenizer.name_or_path
    raw_texts, raw_tags = get_data(lang, kind)
    # print(raw_texts[1-trash], len(raw_texts[1-trash]), '\n', raw_tags[1-trash], len(raw_tags[1-trash]))
    labels = [[tag2id[tag] for tag in doc] for doc in raw_tags]  # 把tags转换映射id（未分词前）
    tokenized_input = tokenizer(raw_texts,
    ...
    if kind == "train" or kind == "valid":
        ...
        if keyword is not None:
            labels_encoding = encode_labels_keyword(labels, tokenized_input, tokenizer, keyword)
        else:
            labels_encoding = encode_labels_offset(labels, tokenized_input)
        return tokenized_input, labels_encoding
    elif kind == "test":
...
def get_true_labels(lang, kind):
    _, tags = get_data(lang, kind)
